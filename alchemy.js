const dataServerAlchemy = require('../utils/dataServerAlchemy');

const cekAlchemy = async (browser, id, server) => {
   if (!id) return { code: 400, message: 'ID Game Tidak Boleh Kosong' };

   const findServer = dataServerAlchemy.filter((item) => item.zoneId == server);

   if (findServer.length < 1) return { code: 404, message: 'Server Tidak Ditemukan' };

   try {
      page = browser.alchemy;
